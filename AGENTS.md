# AGENTS.md — Operating Contract for agent-atlas-growth-department
...
- **One owner per file.** Do not write a file you do not own — including harness
  config. (`F-00-09`: a Phase 00 subagent wrote `.Codex/settings.local.json`
...
|---|---|---|
| `AGENTS.md` | This operating contract; loaded first every session | Only when architecture, boundaries, or vocabularies actually change |
...
- **The finding that changes how later phases must think:** this project's
  ...
  Codex account, connector grants held in that account, harness-mediated egress,
